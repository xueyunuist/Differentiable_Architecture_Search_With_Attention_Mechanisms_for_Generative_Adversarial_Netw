...
class Network_gen_Auto(nn.Module):
  ...
  def __init__(self, args):
    ...
    self._C = args.gf_dim  # 256
    self._num_classes = args.num_classes  # 10（数据集的类数）
    self._bottom_width = args.bottom_width  # 4（通道大小）
    self._use_gumbel = False
    self._args = args
    self._normal_primitives = eval('genotypes.' + args.gen_normal_opr)  # args.gen_normal_opr is PRIMITIVES_NORMAL_GEN_wo_skip_none_sep
    self._up_primitives = eval('genotypes.' + args.gen_up_opr)  # args.gen_up_opr is PRIMITIVES_UP
  ...
  def forward(self, input, eval=False):
    ...
    s1, ft_1, up_Attention_cell1, op_Attention_cell1 = self.cell1(ft_linear)
    s2, ft_2, up_Attention_cell2, op_Attention_cell2 = self.cell2(ft_1, [s1])
    _, ft_3, up_Attention_cell3, op_Attention_cell3 = self.cell3(ft_2, [s1, s2])
    ft_final = self.to_rgb(ft_3)
    
    return ft_final, up_Attention_cell1, up_Attention_cell2, up_Attention_cell3, op_Attention_cell1, op_Attention_cell2, op_Attention_cell3
  ...
  def _initialize_alphas(self):
    ...
    num_ops_norm = len(self._normal_primitives)  # 6
    num_ops_up = len(self._up_primitives)  # 3
    num_ops_up_skip = len(self._up_primitives)  # 3
    self.alphas_normal_1 = Variable(1e-3*torch.randn(self.k_normal, num_ops_norm).cuda(), requires_grad=self._update_alphas)  # k_normal = 3
    ...
    self.alphas_up_1 = Variable(1e-3*torch.randn(self.k_up, num_ops_up).cuda(), requires_grad=self._update_alphas)  # k_up = 2
  ...
  # 找到结构参数最大的边的操作是什么
  def genotype(self, up_attention_cell1, up_attention_cell2, up_attention_cell3, op_attention_cell1, op_attention_cell2, op_attention_cell3):
    up_attention_cell2_up = []
    up_attention_cell2_skip = []
    up_attention_cell2_skip.append(up_attention_cell2[0])
    up_attention_cell2_up.append(up_attention_cell2[1])
    up_attention_cell2_up.append(up_attention_cell2[2])
    up_attention_cell3_up = []
    up_attention_cell3_skip = []
    up_attention_cell3_skip.append(up_attention_cell3[0])
    up_attention_cell3_skip.append(up_attention_cell3[1])
    up_attention_cell3_up.append(up_attention_cell3[2])
    up_attention_cell3_up.append(up_attention_cell3[3])
    ...
    gene_normal = {}
    gene_normal['1'] = _parse(op_attention_cell1, self.k_normal, self._normal_primitives)
    gene_normal['2'] = _parse(op_attention_cell2, self.k_normal, self._normal_primitives)
    gene_normal['3'] = _parse(op_attention_cell3, self.k_normal, self._normal_primitives)
    gene_up = {}
    gene_up['1'] = _parse(up_attention_cell1, self.k_up, self._up_primitives)
    gene_up['2'] = _parse(up_attention_cell2_up, self.k_up, self._up_primitives)
    gene_up['3'] = _parse(up_attention_cell3_up, self.k_up, self._up_primitives)
    gene_skip_2 = _parse(up_attention_cell2_skip, self.skip_2, self._up_primitives)
    gene_skip_3 = _parse(up_attention_cell3_skip, self.skip_3, self._up_primitives)
